...
| Gate Epic 1 | `Blocked` | Epic 0 encore en cours ; Story 0.2 cloturee ; Story 0.3 preparee localement (`steam_appid.txt`, helper smoke test Steamworks, transport Facepunch verifie) avec preuve VAL-012 fournie ; VAL-013 a VAL-015 restent en cours ; Story 0.4 scaffold terminee ; Stories 0.5 a 0.8 encore ouvertes | Epic 1 reste bloquee tant que l'Epic 0 n'est pas marquee `Pass` ou explicitement acceptee avec bloqueurs documentes. |
...
| 0.3 Steamworks et Networking Sockets | `In Progress` | Suivre `docs/setup/story-0-3-unity-cloud-services-tutorial.md` : VAL-012 est prouve par le smoke test AppID 480 ; confirmer ensuite creation de lobby Steam, Networking Sockets et invite/Lobby ID readiness quand le runtime existe. | Notes ou captures caviardees pour VAL-013 a VAL-015, plus exigences VAL-032 ; aucun token, Lobby ID complet, credential ou invite utilisable. | Session privee host-created, Steamworks Networking Sockets, Lobby ID, `MaxPlayers = 4`, absence de port forwarding routeur host, invite Steam/Lobby ID et exigences d'erreurs Lobby/UI visibles sont prouves, bloques ou references vers Story 2.2, Story 2.3 et Story 0.8 quand le runtime n'existe pas encore. | VAL-012 passe avec capture Console Unity 2026-09-03 20:19:46. Preparation locale faite par l'agent : AppID 480, helper smoke test et verification statique du transport. Pas de service payant, matchmaking public, dedicated server, host migration, native deep link, changement de cap joueurs, stockage de token ou dependance externe sans approbation. |
| 0.4 Structure projet et scenes | `Pass` | Aucune action manuelle restante pour le scaffold Story 0.4 ; ouvrir Unity seulement pour revue visuelle si souhaitee. | `Assets/RoadRage`, scenes sous `Assets/RoadRage/App/Scenes`, `ProjectSettings/EditorBuildSettings.asset`, tests EditMode `RoadRageScaffoldTests`. | Scenes seed, structure `Assets/RoadRage`, namespaces `RoadRage.App`, `RoadRage.Shared`, `RoadRage.Features.<Feature>` et squelette runtime state sont inspectables. | Scaffold cree sans gameplay ; les scenes dev restent desactivees dans les build settings. |
...
| 0.4 Structure et squelette runtime | `Pass` | Verifier dossiers `Assets/RoadRage`, scenes, namespaces, asmdefs et noms de squelettes host-owned runtime state. | `Assets/RoadRage`; `ProjectSettings/EditorBuildSettings.asset`; `RoadRageScaffoldTests` execute via Unity batchmode ; VAL-016 a VAL-019. | La structure respecte le seed et les limites d'assemblies sans commencer Epic 1. | `RoadRage.App`, `RoadRage.Shared` et `RoadRage.Features.<Feature>` sont presents ; aucune reference directe feature-to-feature ajoutee ; confirme par execution reelle de `RoadRageScaffoldTests` (6/7 tests verts, compilation Unity sans erreur). |
