...
| Gate Epic 1 | `Blocked` | Epic 0 encore en cours ; Story 0.2 cloturee ; Story 0.3 preparee pour revue documentaire mais preuves manuelles non fournies ; Stories 0.4 a 0.8 encore ouvertes | Epic 1 reste bloquee tant que l'Epic 0 n'est pas marquee `Pass` ou explicitement acceptee avec bloqueurs documentes. |
...
| 0.2 Packages verrouilles | `In Progress` | Suivre `docs/setup/story-0-2-unity-install-tutorial.md` (etape 4) : installer ou confirmer via Package Manager : Universal Render Pipeline `17.6.0`, Netcode for GameObjects `2.13.2`, transport Steamworks (git URL, remplace Multiplayer Services 2.3.1 retire), Unity Transport `6.6.0`, Multiplayer Play Mode `3.0.0`, Input System `1.20.0` et Cinemachine `6.6.0`. | `Packages/manifest.json` et `Packages/packages-lock.json` apres resolution Unity, un extrait par package comme demande a l'etape 4 du tutoriel. | Tous les packages verrouilles sont presents aux versions/commits attendus ou chaque ecart est documente et accepte avant Epic 1. | Course-correction (`sprint-change-proposal-2026-09-02.md`) : VAL-007 repassee `Not Started`, retrait manuel de Multiplayer Services et installation du transport Steamworks encore a faire. |
| 0.3 Steamworks et Networking Sockets | `In Progress` | Suivre `docs/setup/story-0-3-unity-cloud-services-tutorial.md` : configurer l'AppID de test Steamworks 480 (Spacewar), installer le transport Steamworks, confirmer creation de lobby Steam, Networking Sockets et invite/Lobby ID readiness. | Notes ou captures caviardees pour VAL-012 a VAL-015, plus exigences VAL-032 ; aucun token, Lobby ID complet, credential ou invite utilisable. | Session privee host-created, Steamworks Networking Sockets, Lobby ID, `MaxPlayers = 4`, absence de port forwarding routeur host, invite Steam/Lobby ID et exigences d'erreurs Lobby/UI visibles sont prouves, bloques ou references vers Story 2.2, Story 2.3 et Story 0.8 quand le runtime n'existe pas encore. | Course-correction (`sprint-change-proposal-2026-09-02.md`) : story reciblee Steamworks, aucune preuve Unity Cloud n'existait donc rien a defaire. Pas de service payant, matchmaking public, dedicated server, host migration, native deep link, changement de cap joueurs, stockage de token ou dependance externe sans approbation. |
...
Ne commence pas le gameplay Epic 1 tant que le setup Unity, les pins packages, Unity services, la securite MCP, l'asset intake Blender, la revue d'adoption add-on ou les smoke tests requis sont encore non documentes.
